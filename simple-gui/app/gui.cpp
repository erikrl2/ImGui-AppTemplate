...
long __stdcall WindowProcess(HWND window, UINT message, WPARAM wideParameter, LPARAM longParameter) { // window process that handles window events
	...
	case WM_SYSCOMMAND: {
		if (wideParameter & 0xfff0) { // Disable ALT application menu
			return 0;
		} break;
	}
...
void gui::CreateHWindow(const char* windowName, const char* className) noexcept {
	...
	window = CreateWindow(className, windowName, WS_POPUP, 100, 100, WIDTH, HEIGHT, 0, 0, windowClass.hInstance, 0); // create window
...
void gui::Render() noexcept {
	...
	ImGui::Begin("Sample Menu", &isRunning, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove);
